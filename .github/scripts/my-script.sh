...
for file in $propfiles; do
  ...
  diff=$(git diff --unified=0  origin/dev HEAD -- $file  | grep -Ev '(index|@@|--git|---|\+\+\+|new)' | sed -E ':a;N;$!ba;s/\r{0,1}\n/\\n/g' )
  fileM=$(git diff --name-status  origin/dev HEAD -- $file)
  date=$(date '+%Y-%m-%d %H:%M:%S')
  sep1="____________________________________________\n"
  sep2='\n--------------------------------------------\n'
  echo -e '\n***********>>>>>'$file
  
  echo $(git diff --name-status origin/dev HEAD -- $file)
  echo -e '\n***********>>>>>'
  if [ -f "$file" ] && [ "$diff" ]
  then  
